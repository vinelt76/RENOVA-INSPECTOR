-- RENOVA — índice unificado para el buscador global de unidades y cascos.
--
-- La vista parte únicamente de tablas base. `security_invoker` hace que las
-- políticas RLS de cada tabla mantengan el aislamiento por empresa; por eso no
-- se agrega un filtro manual por company_id aquí.
...
select
  ...
  case
    ...
    -- Un casco activo sin ciclo vigente viola el invariante de ciclo de vida.
    -- Se conserva visible, pero sin asignarle un badge de negocio falso.
    else null
  end as status,
...
comment on view public.v_search_index is
  'Índice de búsqueda de unidades y cascos. Una fila por objeto; las tablas base y security_invoker preservan RLS por empresa.';
