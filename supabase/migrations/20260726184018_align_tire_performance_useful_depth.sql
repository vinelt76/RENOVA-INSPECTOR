-- Alinea la implementación SQL con el contrato de Rendimiento:
-- la vida disponible parte del RTD al instalar, no del OTD del ciclo.
-- El costo por km conserva por ahora su definición vigente (costo del ciclo /
-- km acumulado del ciclo) hasta que RENOVA confirme D6.
--
-- Cambio histórico: consumption_pct se recalcula al consultar la vista. No hay
-- datos derivados almacenados, pero capturas/reportes previos pueden diferir.
...
select
  ...
  case
    when (k.rtd_at_install_mm - k.last_inspection_rtd_mm) > 0
      and k.km_run is not null
      and (k.rtd_at_install_mm - rt.rtd_removal_mm) > 0
    then k.km_run::numeric
      / (k.rtd_at_install_mm - k.last_inspection_rtd_mm)
      * (k.rtd_at_install_mm - rt.rtd_removal_mm)
  end as km_projected,
...
comment on view public.v_tire_performance is
  'Métricas por instalación activa. Consumo y km proyectado usan la profundidad útil (RTD al instalar menos umbral de retiro), nunca el OTD. Costo/km conserva costo del ciclo / km del ciclo hasta resolver D6.';
