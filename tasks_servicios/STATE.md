...
> **Resolución posterior (2026-07-22):** la tabla sigue fuera de `supabase_realtime`, pero
> `servicios-controller.js` ya combina la suscripción con un fallback de lectura silenciosa al
> enfocar/volver a la pestaña y cada 10 segundos visible. `task_08` conserva su estado histórico;
> para cerrar la evidencia falta repetir el punto 17 con una orden real tras sanear los datos.
...
| 12 | Suites y prueba de campo | CODEX + USUARIO | APROBADO | 11 | `PRUEBA_CAMPO_PAREO.md` | Ejecutada 2026-07-22 sobre MÓVIL BUS 2145 mediante las interfaces y RPC normales. Rotación `cd5c27db-0d1d-4fd9-96f2-5ebec48223fc`: 4 `request_items`, 4 ejecuciones, 2 servicios `rotation` con pareo `exact`; P3 recibe `25311` desde P4 y P4 recibe `25324` desde P3, sin casco de la pareja sin salida. Scrap con reemplazo `b47fd97b-2d20-415a-9eaf-03e5858b49e3`: 2 ejecuciones y exactamente 1 servicio `discard`, sin `installation` fantasma. La app operaria manejó 4 renglones sin cambios. Servicios 0→2→3 sin reload manual en 7,2 s y 7,4 s. Consola/excepciones 0. Suites repetidas: Servicios 38/38, Movimientos 186/186, app operaria 3/3; matriz integral previa 380/380. Los `N/A` no críticos y su cobertura están documentados. Las 6 ejecuciones quedan `reconciliation_status=pending`, conforme al contrato vigente. |
...
**Corrección UX posterior a la prueba de campo (2026-07-22).** La frase histórica de la fila 12
«4 renglones sin cambios» describe lo que se probó inicialmente, no la interfaz vigente. La app
ahora muestra **2 tarjetas de servicio** para esa rotación; cada tarjeta contiene `SALE` con su razón
y `ENTRA` con sus datos y origen. Internamente conserva las 4 capturas y el payload 1:1. No hubo
cambios de tabla, RPC, policy ni vista por esta corrección.
...
2. **Cero cambios en el contrato de `app movimientos/`.** La corrección UX posterior a campo
   agrupa las 4 capturas técnicas en 2 servicios por posición, sin cambiar RPC, esquema ni
   cardinalidad. Si no conserva ambas salidas y ambas entradas (`task_12` punto 3), se detiene todo.
