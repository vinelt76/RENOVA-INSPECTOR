...
function legacyDraftKey(scope) {
  return `${LEGACY_STORE_PREFIX}:draft:${scopeKey(scope)}`;
}
...
function legacySealedPrefix(scope) {
  return `${LEGACY_STORE_PREFIX}:sealed:${scopeKey(scope)}:`;
}

function removeLegacySealed(scope, storage) {
  for (const key of listKeys(storage)) {
    if (key.startsWith(legacySealedPrefix(scope))) removeValue(key, storage);
  }
}
...
function migrateEnvelope(legacyKey, key, kind, scope, storage) {
  const value = readEnvelope(legacyKey, kind, scope, storage);
  if (value == null) return null;

  writeValue(key, serializeEnvelope(kind, scope, value), storage);
  removeValue(legacyKey, storage);
  return value;
}
...
export function loadDraft(scope, storage) {
  const normalizedScope = normalizeScope(scope);
  const key = draftKey(normalizedScope);
  const current = readEnvelope(key, "draft", normalizedScope, storage);
  if (current != null) {
    removeValue(legacyDraftKey(normalizedScope), storage);
    return current;
  }
  return migrateEnvelope(
    legacyDraftKey(normalizedScope),
    key,
    "draft",
    normalizedScope,
    storage,
...
export function loadSealed(scope, storage) {
  ...
  for (const key of candidates) {
    ...
    removeLegacySealed(normalizedScope, storage);
    return deepFreeze(payload);
  }

  const legacyPrefix = legacySealedPrefix(normalizedScope);
  const legacyCandidates = listKeys(storage)
  ...
  for (const key of legacyCandidates) {
    ...
    const currentKey = sealedKey(normalizedScope, payload.batch_id);
    migrateEnvelope(key, currentKey, "sealed", normalizedScope, storage);
    return deepFreeze(payload);
  }
...
export function clearSealed(batchId, storage) {
  ...
  for (const key of listKeys(storage)) {
    if (
      (key.startsWith(`${STORE_PREFIX}:sealed:`) ||
        key.startsWith(`${LEGACY_STORE_PREFIX}:sealed:`)) &&
      key.endsWith(suffix)
    ) {
      removeValue(key, storage);
    }
