...
export function createMovementsUI({
  ...
  copy.dataset.task = "task-10-movements";
  ...
  feedback.id = "movimientos-movement-feedback";
  feedback.setAttribute("role", "status");
  feedback.setAttribute("aria-live", "polite");

  container.append(copy, actions, feedback);
  ...
  if (discardTitle) {
    discardTitle.id ||= "movimientos-discard-title";
  ...
  function render(state = getState()) {
    ...
    if (row.is_empty === true) {
      copy.textContent = "DISPONIBLE PARA MONTAJE";
      actions.append(createButton(documentObject, "mount", "Elegir para montar"));
      return;
    }

    copy.textContent = "CICLO ACTUAL CONSERVADO EN EL BORRADOR";
    actions.append(
