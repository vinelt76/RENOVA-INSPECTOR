...
function buildDockButton(documentObject, position, row, visualState) {
  ...
  number.className = "tc-position-number num";
  number.textContent = `P${position}`;
  ...
  label.className = "tc-position-label";
  label.textContent = visualState.label;
  text.append(identity, label);
...
export function createDiagramView({
  ...
  function onWheelClick(event) {
    ...
    // propagación en captura evita que cambie POSICIONES/selected en Cambios.
    event.preventDefault();
